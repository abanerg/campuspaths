...
/**
 * This represents an immutable path between two data nodes of type {@code N}, particularly
...
public class Path<N> implements Iterable<Path<N>.Segment> {
    ...
    // AF(this) =
    //      first data node in the path => start
    //      each "step" along the path between data node => elements of list path, where
    //          path.get(0) is the first step from the start data node to an intermediate data node, and
    //          path.get(path.size() - 1) is the final step from an intermediate data node to the end
    //      total cost along the path => cost
    //      the destination data node in this path, opposite the start data node => getEnd()
    ...
    /**
     * The data node at the beginning of this path.
     */
    private N start;

    /**
     * The ordered sequence of segments representing a path between data nodes.
     */
    private List<Path<N>.Segment> path;

    /**
     * Creates a new, empty path containing a start data node. Essentially this represents a path
     * from the start data node to itself with a total cost of "0".
     *
     * @param start The starting data node of the path.
     */
    public Path(N start) {
        this.start = start;
        this.cost = 0;
    ...
    /**
     * Appends a new single segment to the end of this path, originating at the current last data node
     ...
     * segmentCost}.
     *
     * @param newEnd      The data node being added at the end of the segment being appended to this path
    ...
    /**
     * @return The data node at the beginning of this path.
     */
    public N getStart() {
        return start;
    }
    ...
    /**
     * @return The data node at the end of this path, which may be the start data node if this path
     * contains no segments (i.e. this path is from the start data node to itself).
     */
    public N getEnd() {
        if(path.size() == 0) {
            return start;
     ...
     * data node and ending at the end data node. In the case that this path represents a path between
     * the start data node and itself, this iterator contains no elements. This iterator does not
    ...
    public Iterator<Path<N>.Segment> iterator() {
        ...
        return new Iterator<Path<N>.Segment>() {
     ...
     * both paths are empty, they are only equal if their starting data node is equal.
     *
    ...
    /**
     * Segment represents a single segment as part of a longer, more complex path between data node.
    ...
    public class Segment {
         ...
         *
         * @param start The starting data node of this segment.
         * @param end   The ending data node of this segment.
         * @param cost  The cost of travelling this segment.
         * @throws NullPointerException     if either data node is null.
         * @throws IllegalArgumentException if cost is infinite or NaN
         */
        private Segment(N start, N end, double cost) {
            if(start == null || end == null) {
                throw new NullPointerException("Segments cannot have null data nodes.");
            }
        ...
        /**
         * @return The beginning data node of this segment.
         */
        public N getStart() {
        ...
        /**
         * @return The ending data node of this segment.
         */
        public N getEnd() {
            return this.end;
        }
